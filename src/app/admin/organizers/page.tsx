  ...
  return (
    <div>
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-black/70 backdrop-blur-2xl">
        <div className="flex h-20 items-center justify-between px-6 lg:px-10">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black lg:text-3xl">Vérification organisateurs</h1>
          </div>
      ...
      <div className="mx-auto max-w-[1400px] p-6 pb-24 lg:p-10 lg:pb-10">
        ...
        <div className="mb-6 flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-3">
        ...
        <div className="mb-8 flex flex-wrap gap-3">
          {([
            { key: "pending", label: "En attente", count: counts.pending, cls: "bg-yellow-600" },
            { key: "verified", label: "Vérifiés", count: counts.verified, cls: "bg-green-600" },
            { key: "all", label: "Tous", count: counts.all, cls: "bg-[#FF5A1F]" },
          ] as const).map((f) => (
            <button
              key={f.key}
              onClick={() => setFilter(f.key)}
              className={`flex items-center gap-2 rounded-2xl px-5 py-3 font-bold transition ${filter === f.key ? f.cls : "bg-white/[0.05] text-zinc-400 hover:text-white"}`}
            >
              {f.label}
        ...
        {!loading && filtered.length === 0 && (
          <div className="rounded-[32px] border border-dashed border-white/10 p-16 text-center">
            <Building2 size={40} className="mx-auto text-zinc-700" />
            <h2 className="mt-6 text-2xl font-black">{filter === "pending" ? "Aucun organisateur en attente" : "Aucun résultat"}</h2>
          </div>
        )}

        <div className="space-y-5">
          {filtered.map((o) => (
            <div key={o.id} className="overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.03]">
              <div className="flex flex-col gap-6 p-6 sm:flex-row sm:items-center lg:p-8">
                ...
                <div className="flex items-center gap-4 sm:w-64 sm:shrink-0">
                  ...
                  <div className="min-w-0">
                    <p className="truncate font-black">{o.full_name || "Sans nom"}</p>
                    <p className="truncate text-sm text-zinc-400">{o.email}</p>
                    ...
                    <p className="mt-1 text-xs text-zinc-600">Inscrit le {new Date(o.created_at).toLocaleDateString("fr-FR")}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-1 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="space-y-2">
                    ...
                    <span className={`inline-flex rounded-full px-4 py-2 text-xs font-bold uppercase tracking-[0.15em] ${
                      o.organizer_verified ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"
                    }`}>
                    ...
                    {o.organizer_doc_url && (
                      <div>
                        <a
                          ...
                          className="mt-1 inline-flex items-center gap-2 rounded-xl border border-[#FF5A1F]/20 bg-[#FF5A1F]/5 px-4 py-2 text-xs font-bold text-[#FF5A1F] transition hover:bg-[#FF5A1F]/10"
                        >
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className={`flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold transition ${
                        o.organizer_verified ? "cursor-not-allowed bg-zinc-800 text-zinc-600" : "bg-green-600 hover:scale-105"
                      }`}
                    >
                    ...
                    <button
                      ...
                      disabled={!o.organizer_verified}
                      className={`flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold transition ${
                        !o.organizer_verified ? "cursor-not-allowed bg-zinc-800 text-zinc-600" : "bg-red-600 hover:scale-105"
                      }`}
                    >
